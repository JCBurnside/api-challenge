const API_KEY="5e608d12772d3c6632c55c940c54292d",
	...
	OUTPUT_FORMAT_LYRIC="\
			...
			<h5>lyrics</h5><p>%s</p>",
	OUTPUT_FORMAT_LIST_SONGS="<li>%s</li>";
$(()=>{
	...
	$('#submit').click(()=>{
		...
		if(!song){
			...
			.done((res)=>{
				...
				if (titles.length > 0) {
					$output.append("<ul>");
					titles.forEach(element=>$output.find('ul').append(OUTPUT_FORMAT_LIST_SONGS.format(element.title||'ERROR')));
					$output.append("</ul>");
				}else{
					$output.append("<h3 style='color:red;'>ARTIST NOT FOUND</h3>")
				}
			})
		}else{console.log(API_FORMAT_URL.format(API_FORMAT_LYRIC.format(artist,song,API_KEY)));
			$.ajax({
				type:"GET",
